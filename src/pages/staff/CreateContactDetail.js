...
const CreateContactDetail = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (type === "" || number === "" || contact === "") {
      alert("Please fill complete information", { autoClose: 1000 });
      return;
    }
    ...
    if (number.length !== 10) {
      alert("Number should be of 10 digits only", { autoClose: 1000 });
      return;
    }
    ...
    if (resp) {
      console.log(resp.data);
      alert("Contact Detail created successfullly", {
        autoClose: 1000,
      });
      return;
    }
